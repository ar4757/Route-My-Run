...
class FirstViewController: UITableViewController {
    ...
    func getRuns() {
        ...
        let connection = GraphRequestConnection()
         connection.add(GraphRequest(graphPath: "/me/friends", parameters: ["fields": ""], accessToken: AccessToken.current, httpMethod: GraphRequestHTTPMethod(rawValue: "GET")!, apiVersion: "2.8")) { httpResponse, result in
         print("result == ", result)
         switch result {
         case .success(let response):
            let friendsArray = response.dictionaryValue!["data"] as! NSArray
            print("Friends are: \(friendsArray)")
            for friend in friendsArray {
                UserProfile.fetch(userId: (friend as! Dictionary)["id"]!, completion: { result in
                    switch result {
                    case .success(let response):
                        ...
                        db.collection("users").document(userId).collection("runs").order(by: "timestamp", descending: true).getDocuments() { (querySnapshot, err) in
                            if let err = err {
                                ...
                                for document in querySnapshot!.documents {
                                    ...
                                    newRun.name = response.firstName
                                    ...
                                    newRun.profilePictureURL = response.imageURLWith(UserProfile.PictureAspectRatio.normal, size: CGSize.init(width: 80, height: 80))
        ...
        db.collection("users").document(userId!).collection("runs").order(by: "timestamp", descending: true).getDocuments() { (querySnapshot, err) in
            if let err = err {
                ...
                for document in querySnapshot!.documents {
                    ...
                    newRun.time = document.get("time") as! Int
                    newRun.pace = document.get("pace") as! Double
                    newRun.profilePictureURL = UserProfile.current?.imageURLWith(UserProfile.PictureAspectRatio.normal, size: CGSize.init(width: 80, height: 80))
                    self.runs.append(newRun)
                    self.runs.sort(by: { (RunObject1, RunObject2) -> Bool in
                        if (RunObject1.timestamp > RunObject2.timestamp) {
                            return true
